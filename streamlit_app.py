...
client_type = st.radio(
    "Izvēlieties klienta import streamlit as st
import numpy as np
...
if is_business:
    ...
    # Privātpersonām fiksēts 6500 EUR atbalsts
    fixed_grant = 6500
    grant_pct = 0
...
calc_battery = calc_solar * 1.0 

# --- CENU KALIBRĒŠANA (Lai 14kW sistēma ar PVN būtu ~14k EUR) ---
# Ja calc_solar ir 14, tad (14 * sol_price + 14 * bat_price) * 1.21 = 14000
# Tas nozīmē, ka bāzes cenai jābūt ap 825 EUR/unit (saules + bat kombinācija)
if calc_solar < 20:
    sol_price_base, bat_price_base = 650, 200  # Kopā 850 * 1.21 = ~1028 (pie 14kW = 14.3k EUR)
elif calc_solar < 50:
    sol_price_base, bat_price_base = 600, 180
else:
    sol_price_base, bat_price_base = 550, 150  # Industriālā cena (bez PVN)
...
if is_business:
    ...
    # Privātpersonai vienmēr 6500 (bet ne vairāk kā projekta cena)
    grant_amount = min(fixed_grant, total_cost)
...
if submit_button or usage:
    st.divider()
    st.subheader(f"📊 Piedāvājums: {client_type}")
    ...
    if monthly_net_profit > 0:
        st.success("✅ Projekts ir pašfinansējošs (ietaupījums > kredīts).")
    
    st.area_chart([(total_savings_monthly * 12 * y) - net_investment for y in range(int(max(payback_years+3, 5)))])tipu:",
    ["Juridiska persona (Bez PVN)", "Privātpersona (Ar PVN 21%)"],
    horizontal=True
)
